...
该 TinyML 系统直接在嵌入式硬件上使用机器学习实时检测动作。系统采用名为 MPU6500 的六轴传感器来测量运动和旋转，目前针对 X 轴和 Y 轴运动进行了优化。

## 功能特点

*   **实时检测**：摔倒检测响应时间为 150-300ms
...
*   **平台**：运行 QuecPython 的 Quectel 嵌入式模块
...
```
   ...
   2.**烧录固件：** 按照[说明](https://developer.quectel.com/doc/quecpython/Getting_started/zh/4G/flash_firmware.html)将固件烧录到开发板上。
...
系统会自动实时检测姿势。当识别到姿势时，会输出分类结果（0、1、2 ）并清除所有缓冲区，以防止来自先前姿势的干扰。
